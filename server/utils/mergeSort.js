...
async function connectDB() {
	const usersCollection = db.collection(Collection.SHARKS);
	try {
		const sharks = await sharksCollection.find({}).toArray();
		for (const shark of sharks) {
			Node.insert(shark.sharkID, shark.originLat, shark.originLong, shark.speed, shark.active, shark.size, shark.weight, shark.gender, shark.dateOfBirth, shark.speciesI);
		}
...
async function main() {
	...
	sortListByChoice("dateOfBirth");
	console.log("\n");
	console.log("species");
	sortListByChoice(2);
	//const filteredHead = filterListBySpecies(3);
	console.log("species");
	await sortListByChoice("speciesID");
